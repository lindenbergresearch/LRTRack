...
void Westcoast::step() {
    ...
    hs->setShape(params[KPOS_PARAM].value, params[KNEG_PARAM].value);
...
void Westcoast::onSampleRateChange() {
    ...
    // pass to dsp system
    hs->updateSampleRate(engineGetSampleRate());
}
...
WestcoastWidget::WestcoastWidget(Westcoast *module) : LRModuleWidget(module) {
    ...
    // ***** MAIN KNOBS ******
    addParam(LRKnob::create<LRBigKnob>(Vec(102, 64.9), module, Westcoast::GAIN_PARAM, 1, 5.f, 1.f));
